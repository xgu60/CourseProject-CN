...
class DistanceVector(Node):
    ...
    def process_BF(self):
        ...
        for msg in self.messages:
            # TODO: Do something
            for key in msg.keys():
                tempDistance = 1 + msg[key]                          
                if key not in self.dv.keys():
                    self.dv[key] = tempDistance
                    needUpdate = True                
                elif self.dv[key] > tempDistance:
                    self.dv[key] = tempDistance
                    needUpdate = True
        ...
        if needUpdate:
            for neighbor in self.links:
                self.send_msg(self.dv, neighbor)
    ...
    def log_distances(self):
        ...
        switch = self.name
        logstring = ""
        for key in sorted(self.dv):
            logstring += key + str(self.dv[key]) + ","
